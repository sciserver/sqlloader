...
DROP TABLE IF EXISTS dbo.mos_allwise
...
DROP TABLE IF EXISTS dbo.mos_assignment
...
DROP TABLE IF EXISTS dbo.mos_best_brightest
...
DROP TABLE IF EXISTS dbo.mos_bhm_csc
...
DROP TABLE IF EXISTS dbo.mos_bhm_csc_v2
...
DROP TABLE IF EXISTS dbo.mos_bhm_efeds_veto
...
DROP TABLE IF EXISTS dbo.mos_bhm_rm_tweaks
...
DROP TABLE IF EXISTS dbo.mos_bhm_rm_v0
...
DROP TABLE IF EXISTS dbo.mos_bhm_rm_v0_2
...
DROP TABLE IF EXISTS dbo.mos_bhm_spiders_agn_superset
...
--/T [public]ly available eROSITA catalogs. The dr19_bhm_spiders_*_superset tables are
...
----------------------------------------------------------------------
    ...
    unwise_dr1_objid varchar(500), --/D Identifier of counterpart (if any) in dr19_unwise (not used) 
...
DROP TABLE IF EXISTS dbo.mos_bhm_spiders_clusters_superset
...
DROP TABLE IF EXISTS dbo.mos_cadence
...
DROP TABLE IF EXISTS dbo.mos_cadence_epoch
...
DROP TABLE IF EXISTS dbo.mos_carton
...
----------------------------------------------------------------------
    ...
    target_selection_planname varchar(500) --/D The target selection planname version for which this carton was run. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_carton_csv
...
DROP TABLE IF EXISTS dbo.mos_carton_to_target
...
DROP TABLE IF EXISTS dbo.mos_cataclysmic_variables
...
DROP TABLE IF EXISTS dbo.mos_catalog
...
DROP TABLE IF EXISTS dbo.mos_catalog_to_allstar_dr17_synspec_rev1
...
----------------------------------------------------------------------
    ...
    target_id varchar(500) NOT NULL, --/D The primary key identifier in the dr19_allstar_dr17_synspec_rev1 table. 
    ...
    best bit, --/D Whether this is considered the best match between the catalog entry and dr19_allstar_dr17_synspec_rev1. 
    planname_id varchar(500) --/D Identifier of the cross-matching planname used to generate this file. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_allwise
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_allwise. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_bhm_csc
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_bhm_csc. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_bhm_efeds_veto
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_bhm_efeds_veto. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_bhm_rm_v0
...
----------------------------------------------------------------------
    ...
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_rm_v0. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_bhm_rm_v0_2
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/U deg --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_rm_v0_2. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_catwise2020
...
----------------------------------------------------------------------
    catalogid bigint NOT NULL, --/D The catalogid identifier in the dr19_catalog table. 
    target_id varchar(25) NOT NULL, --/D The primary key identifier in the dr19_catwise2020 table. 
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_catwise2020. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_gaia_dr2_source
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_gaia_dr2_source. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_glimpse
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_glimpse. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_guvcat
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_guvcat. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_legacy_survey_dr8
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_legacy_survey_dr8. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_mangatarget
...
----------------------------------------------------------------------
    catalogid bigint NOT NULL, --/D The catalogid identifier in the dr19_catalog table. 
    target_id varchar(255) NOT NULL, --/D The primary key identifier in the dr19_mangatarget table. 
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit, --/D Whether this is considered the best match between the catalog entry and dr19_mangatarget. 
    planname_id varchar(500), --/D Identifier of the cross-matching planname used to generate this file. 
    added_by_phase smallint --/D The phase of the cross-match in which this target was added. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_marvels_dr11_star
...
----------------------------------------------------------------------
    catalogid bigint NOT NULL, --/D The catalogid identifier in the dr19_catalog table. 
    target_id varchar(255) NOT NULL, --/D The primary key identifier in the dr19_marvels_dr11_star table. 
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit, --/D Whether this is considered the best match between the catalog entry and dr19_marvels_dr11_star. 
    planname_id varchar(500), --/D Identifier of the cross-matching planname used to generate this file. 
    added_by_phase smallint --/D The phase of the cross-match in which this target was added. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_marvels_dr12_star
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit, --/D Whether this is considered the best match between the catalog entry and dr19_marvels_dr12_star. 
    planname_id varchar(500), --/D Identifier of the cross-matching planname used to generate this file. 
    added_by_phase smallint --/D The phase of the cross-match in which this target was added. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_mastar_goodstars
...
----------------------------------------------------------------------
    catalogid bigint NOT NULL, --/D The catalogid identifier in the dr19_catalog table. 
    target_id varchar(255) NOT NULL, --/D The primary key identifier in the dr19_mastar_goodstars table. 
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit, --/D Whether this is considered the best match between the catalog entry and dr19_mastar_goodstars. 
    planname_id varchar(500), --/D Identifier of the cross-matching planname used to generate this file. 
    added_by_phase smallint --/D The phase of the cross-match in which this target was added. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_panstarrs1
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_panstarrs1. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_sdss_dr13_photoobj_primary
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_sdss_dr13_photoobj_primary. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_sdss_dr16_specobj
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_sdss_dr16_specobj. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_sdss_dr17_specobj
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit, --/D Whether this is considered the best match between the catalog entry and dr19_sdss_dr17_specobj. 
    planname_id varchar(500), --/D Identifier of the cross-matching planname used to generate this file. 
    added_by_phase smallint --/D The phase of the cross-match in which this target was added. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_skies_v1
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_skies_v1. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_skies_v2
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_skies_v2. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_skymapper_dr2
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_skymapper_dr2. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_supercosmos
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_supercosmos. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_tic_v8
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_tic_v8. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_twomass_psc
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_twomass_psc. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_tycho2
...
----------------------------------------------------------------------
    catalogid bigint NOT NULL, --/D The catalogid identifier in the dr19_catalog table. 
    target_id varchar(500) NOT NULL, --/D The primary key identifier in the dr19_tycho2 table. 
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_tycho2. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_unwise
...
----------------------------------------------------------------------
    catalogid bigint NOT NULL, --/D The catalogid identifier in the dr19_catalog table. 
    target_id varchar(500) NOT NULL, --/D The primary key identifier in the dr19_unwise table. 
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_unwise. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_uvotssc1
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_uvotssc1. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalog_to_xmm_om_suss_4_1
...
----------------------------------------------------------------------
    ...
    version_id smallint NOT NULL, --/D The internal version for the cross-match. 
    distance double precision, --/D The distance between the catalog and target coordinates if best=F. 
    best bit --/D Whether this is considered the best match between the catalog entry and dr19_xmm_om_suss_4_1. 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_catalogdb_version
CREATE TABLE dbo.mos_catalogdb_version (
----------------------------------------------------------------------
--/H Table containing crossmatch versions
----------------------------------------------------------------------
    id integer NOT NULL, --/D Unique identifier 
    plannamename varchar(500), --/D Cross match version 
    tag varchar(500) --/D Tag of target selection software used https://github.com/sdss/target_selection/tags 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_category
...
DROP TABLE IF EXISTS dbo.mos_catwise2020
...
DROP TABLE IF EXISTS dbo.mos_design
...
----------------------------------------------------------------------
    ...
    design_mode_label varchar(500), --/D The primary key of the design_mode in the dr19_design_mode table. 
...
DROP TABLE IF EXISTS dbo.mos_design_mode
...
DROP TABLE IF EXISTS dbo.mos_design_mode_check_results
...
DROP TABLE IF EXISTS dbo.mos_design_to_field
...
DROP TABLE IF EXISTS dbo.mos_ebosstarget_v5
...
DROP TABLE IF EXISTS dbo.mos_erosita_superset_agn
...
----------------------------------------------------------------------
    ...
    catwise2020_id varchar(25), --/D Identifier of counterpart (if any) in dr19_catwise2020 ('source_id' field) 
    ...
    tycho2_id varchar(12), --/D Identifier of counterpart (if any) in dr19_tycho2 ('designation' field) 
...
DROP TABLE IF EXISTS dbo.mos_erosita_superset_clusters
...
DROP TABLE IF EXISTS dbo.mos_erosita_superset_compactobjects
...
--/T differ from [public]ly available eROSITA catalogs. The dr19_erosita_superset_*
...
DROP TABLE IF EXISTS dbo.mos_erosita_superset_stars
...
DROP TABLE IF EXISTS dbo.mos_field
...
DROP TABLE IF EXISTS dbo.mos_gaia_assas_sn_cepheids
...
DROP TABLE IF EXISTS dbo.mos_gaia_dr2_ruwe
...
DROP TABLE IF EXISTS dbo.mos_gaia_dr2_source
...
DROP TABLE IF EXISTS dbo.mos_gaia_dr2_wd
...
DROP TABLE IF EXISTS dbo.mos_gaia_unwise_agn
...
DROP TABLE IF EXISTS dbo.mos_gaiadr2_tmass_best_neighbour
...
DROP TABLE IF EXISTS dbo.mos_geometric_distances_gaia_dr2
...
DROP TABLE IF EXISTS dbo.mos_glimpse
...
DROP TABLE IF EXISTS dbo.mos_guvcat
...
----------------------------------------------------------------------
    ...
    inlargeobj varchar(500), --/D Is source in the footprint of an extended objectOFF 
    largeobjsize real --/D Size of the extended object 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_hole
...
DROP TABLE IF EXISTS dbo.mos_instrument
...
DROP TABLE IF EXISTS dbo.mos_legacy_catalog_catalogid
CREATE TABLE dbo.mos_legacy_catalog_catalogid (
    catalogid bigint NOT NULL
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_legacy_survey_dr8
...
DROP TABLE IF EXISTS dbo.mos_magnitude
...
DROP TABLE IF EXISTS dbo.mos_mangadapall
...
DROP TABLE IF EXISTS dbo.mos_mangadrpall
...
DROP TABLE IF EXISTS dbo.mos_mangatarget
...
----------------------------------------------------------------------
    ...
    specobjid varchar(500) --/D The associated specObjID from the dr19_sdss_dr17_specobj table 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_mapper
...
DROP TABLE IF EXISTS dbo.mos_marvels_dr11_star
...
----------------------------------------------------------------------
    ...
    twomass_designation varchar(500), --/D 2MASS Designation ID in dr19_twomass_psc 
    tycho2_designation varchar(500) --/D Tycho2 Designation ID in dr19_tycho2 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_marvels_dr12_star
...
DROP TABLE IF EXISTS dbo.mos_mastar_goodstars
...
DROP TABLE IF EXISTS dbo.mos_mastar_goodvisits
...
DROP TABLE IF EXISTS dbo.mos_mipsgal
...
DROP TABLE IF EXISTS dbo.mos_mwm_tess_ob
...
DROP TABLE IF EXISTS dbo.mos_observatory
...
DROP TABLE IF EXISTS dbo.mos_obsmode
...
DROP TABLE IF EXISTS dbo.mos_opsdb_apo_camera
...
DROP TABLE IF EXISTS dbo.mos_opsdb_apo_camera_frame
...
DROP TABLE IF EXISTS dbo.mos_opsdb_apo_completion_status
...
DROP TABLE IF EXISTS dbo.mos_opsdb_apo_configuration
...
DROP TABLE IF EXISTS dbo.mos_opsdb_apo_design_to_status
...
DROP TABLE IF EXISTS dbo.mos_opsdb_apo_exposure
...
DROP TABLE IF EXISTS dbo.mos_opsdb_apo_exposure_flavor
...
DROP TABLE IF EXISTS dbo.mos_panstarrs1
...
DROP TABLE IF EXISTS dbo.mos_positioner_status
...
DROP TABLE IF EXISTS dbo.mos_revised_magnitude
...
DROP TABLE IF EXISTS dbo.mos_sagitta
...
DROP TABLE IF EXISTS dbo.mos_sdss_apogeeallstarmerge_r13
...
DROP TABLE IF EXISTS dbo.mos_sdss_dr13_photoobj_primary
...
DROP TABLE IF EXISTS dbo.mos_sdss_dr16_qso
...
DROP TABLE IF EXISTS dbo.mos_sdss_dr16_specobj
...
DROP TABLE IF EXISTS dbo.mos_sdss_dr17_specobj
...
DROP TABLE IF EXISTS dbo.mos_sdss_id_flat
...
DROP TABLE IF EXISTS dbo.mos_sdss_id_stacked
...
DROP TABLE IF EXISTS dbo.mos_sdss_id_to_catalog
...
DROP TABLE IF EXISTS dbo.mos_sdss_id_to_catalog_full
...
DROP TABLE IF EXISTS dbo.mos_sdssv_boss_conflist
...
DROP TABLE IF EXISTS dbo.mos_sdssv_boss_spall
...
DROP TABLE IF EXISTS dbo.mos_sdssv_plateholes
...
----------------------------------------------------------------------
    ...
    targettype varchar(500), --/D type of target; one of SCIENCE, STANDARD, or SKY (if holetype is BOSS, SDSS, MARVELS or APOGEE) ON [SPEC]; NA if not holetype is not one of the instruments (GUIDE, CENTER, TRAP, ALIGNMENT) 
...
DROP TABLE IF EXISTS dbo.mos_sdssv_plateholes_meta
...
----------------------------------------------------------------------
    ...
    priority varchar(500), --/D order of priority for input files; lower numbers get checked first; if two files have the same priority, their targets are checked simultaneously (in order of the individual priority number for each object) ON [SPEC]; (default is 1..nInputs). 
...
DROP TABLE IF EXISTS dbo.mos_skies_v1
...
DROP TABLE IF EXISTS dbo.mos_skies_v2
...
DROP TABLE IF EXISTS dbo.mos_skymapper_dr2
...
DROP TABLE IF EXISTS dbo.mos_skymapper_gaia
...
DROP TABLE IF EXISTS dbo.mos_supercosmos
...
DROP TABLE IF EXISTS dbo.mos_target
...
DROP TABLE IF EXISTS dbo.mos_targeting_generation
...
--/T targeting_generation with a robostrategy planname, join dr19_targeting_generation to
...
DROP TABLE IF EXISTS dbo.mos_targeting_generation_to_carton
...
DROP TABLE IF EXISTS dbo.mos_targeting_generation_to_version
...
----------------------------------------------------------------------
    generation_pk integer, --/D primary key of an entry in the dr19_targeting_generation table 
    version_pk integer, --/D primary key of an entry in the dr19_targetdb_version table, which lists the robostrategy run version ('planname' and 'tag') 
    pk integer NOT NULL --/D primary key for this table entry 
) ON [SPEC];


DROP TABLE IF EXISTS dbo.mos_tess_toi
...
DROP TABLE IF EXISTS dbo.mos_tess_toi_v05
...
DROP TABLE IF EXISTS dbo.mos_tic_v8
...
DROP TABLE IF EXISTS dbo.mos_twomass_psc
...
DROP TABLE IF EXISTS dbo.mos_tycho2
...
DROP TABLE IF EXISTS dbo.mos_unwise
...
DROP TABLE IF EXISTS dbo.mos_uvotssc1
...
DROP TABLE IF EXISTS dbo.mos_xmm_om_suss_4_1
...
DROP TABLE IF EXISTS dbo.mos_yso_clustering
...
DROP TABLE IF EXISTS dbo.mos_zari18pms
