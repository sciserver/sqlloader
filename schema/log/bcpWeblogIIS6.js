...
function remap(inLine) {
    var ans = "";
    var next = "";
    var method, url, browser;
    var re = /[^ ]+/g;
    var code = /^\d{3}$/g;
    if ((next = re.exec(inLine)) == null) return "" ;
    if ((next = re.exec(inLine)) == null) return "" ;
    ans = ans +        next;   						// hh
    if ((next = re.exec(inLine)) == null) return "" ;			// server ipaddr
    if ((next = re.exec(inLine)) == null) return "" ;   		// method: get, post,...
    method = next;
    ...
    if ((next = re.exec(inLine)) == null) return "" ;			// server port
    if ((next = re.exec(inLine)) == null) return "" ;			// user name
    if ((next = re.exec(inLine)) == null) return "" ;			// client ipaddr
    ans = ans + "\t" + next;						
    ans = ans + "\t" + method;						
    ans = ans + "\t" + url;						
    if ((next = re.exec(inLine)) == null) return "" ;   		// browser
    browser = next;
    if ((next = re.exec(inLine)) == null) return "" ;   		// error code
    ans = ans + "\t" + next; 
    ans = ans + "\t" + browser; 
    return ans;
} 
