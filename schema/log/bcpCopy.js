...
function LaunchBcp(oFile ){
    ...
    var sCmdLine = 'bcp weblog.dbo.weblogTemp in c:\\temp\\bcp.tmp -S localhost -T -c';
...
function MakeLoadFile(oFile){
    var count = 0;
    var buf
    var inStream = oFso.OpenTextFile(oFile.Path);
    if(!inStream.AtEndOfStream) inStream.ReadLine();
    if(!inStream.AtEndOfStream) inStream.ReadLine();
    var line = "";
    if(!inStream.AtEndOfStream) line = inStream.ReadLine();
	...
	if ((line.indexOf('#') != 0) & (line.length > 20) & (line.length < 200)){
		line = remap(line);  
