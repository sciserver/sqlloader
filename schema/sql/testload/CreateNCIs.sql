...
declare CursorIndex cursor for
 ...
 + case when ix.ignore_dup_key=1 then 'IGNORE_DUP_KEY = ON, ' else 'IGNORE_DUP_KEY = OFF, ' end
  + case when sp.data_compression<>0 then 'DATA_COMPRESSION = ' +sp.data_compression_desc +', ' end
 + 'SORT_IN_TEMPDB = ON, FILLFACTOR =' + CAST(ix.fill_factor AS VARCHAR(3)) AS IndexOptions
 , ix.is_disabled , FILEGROUP_NAME(ix.data_space_id) FileGroupName
 from sys.tables t 
 inner join sys.indexes ix on t.object_id=ix.object_id
 inner join sys.partitions sp on sp.object_id=ix.object_id and sp.index_id = ix.index_id
