...
declare CursorIndex cursor for
 ...
 + case when sp.data_compression<>0 then 'DATA_COMPRESSION = ' +sp.data_compression_desc +', ' else +'' end
 + 'SORT_IN_TEMPDB = ON, FILLFACTOR = 100'  AS IndexOptions  --had to do this because it puts 0 for fillfactor sometimes
 , FILEGROUP_NAME(ix.data_space_id) FileGroupName
 from sys.tables t 
 inner join sys.indexes ix on t.object_id=ix.object_id
 inner join sys.partitions sp on sp.object_id=t.object_id and sp.index_id=ix.index_id
