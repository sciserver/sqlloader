...
/****** Object:  StoredProcedure [dbo].[spIndexCreate2]    Script Date: 11/10/2016 2:04:48 PM ******/
SET ANSI_NULLS ON
GO
...
--
CREATE PROCEDURE [dbo].[spIndexCreate_ccs](
	@taskID	int,
	@stepID int,
...
AS BEGIN
	...
	SELECT @type=type, @code=code, @tablename=tableName, 
		@fieldlist=fieldList, @foreignkey=foreignKey, @compression=[compression]
...
*/
	...
	IF ((lower(@type) = 'index') or (lower(@type) = 'unique index'))
		...
		--
		set @msg = @type+' '+@tableName+'('+@fieldList+')'  + ' ' + @compression
