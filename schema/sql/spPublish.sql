...
AS BEGIN
	...
	exec @err = spCopyATable @taskid, @stepID, @fromDB, @toDB, 'mangaDAPall', @firstTime 
	set @summary = @summary + @err 
...
IF EXISTS (SELECT name FROM   sysobjects 
    WHERE  name = N'spPublishNSA' AND  type = 'P')
    DROP PROCEDURE spPublishMastar
GO
--
CREATE PROCEDURE spPublishMastar(
	@taskID int, 
	@stepID int,
	@fromDB varchar(100), 
	@toDB varchar(100), 
...
AS BEGIN
	...
	set @message = 'Starting spPublishMastar';
	exec spNewPhase @taskID, @stepID, 'spPublishMastar', 'OK', @message;

	exec @err = spCopyATable @taskid, @stepID, @fromDB, @toDB, 'mastar_goodstars', @firstTime 
	set @summary = @summary + @err 
	...
	if  @summary = 0 set @status = 'OK' else set @status = 'ABORTING' 
	--
	exec spNewPhase @taskID, @stepID, 'spPublishMastar', @status, 'Published Mastar Tables';
	--
	return @summary
END   -- END spPublishMastar
GO
...
BEGIN
   ...
   -- SET @logfilename = '\\'+@hostname+'\'+REPLACE(@logfilename,':','$');
    ...
    IF @type in ('nsa') 
     ...
     --------------------------------------------------------------------------------------------- 
    -- Handle Mastar databases 
    IF @type in ('mastar') 
        begin 
        exec @err = spPublishMastar @taskID, @stepID, @DBname, @publishDB, @firstTime 
        if @err = 0 
                begin 
                set @stepMsg =  'Published Mastar ' + @type + ' from: ' + @fromDB + ' to: ' + @toDB 
                set @phaseMsg = 'Published Mastar ' + @type + ' from: ' + @fromDB + ' to: ' + @toDB 
                end 
        else 
                begin 
                set @stepMsg =  'Failed to publish Mastar ' + @type + ' from: ' + @fromDB + ' to: ' + @toDB 
                set @phaseMsg = 'Failed to publish Mastar ' + @type + ' from: ' + @fromDB + ' to: ' + @toDB 
                end 
        goto commonExit 
        end 
