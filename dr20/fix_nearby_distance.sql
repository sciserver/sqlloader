...
-- '@nx-(' is the reliable signal: the broken form opens a subexpression there,
-- as in power(@nx-( COS([deccat]) * COS(racat) ),2), while the correct form
-- reads power(@nx-cx,2). An earlier version of this check tested
--   definition LIKE '%COS([%' AND definition NOT LIKE '%@nx-cx%'
-- which cannot work: the broken functions still contain '@nx-cx' in their
-- WHERE and ORDER BY clauses, so it returned 0 on a fully broken database.
SELECT o.name AS still_broken
FROM sys.sql_modules m JOIN sys.objects o ON o.object_id=m.object_id
WHERE o.name LIKE 'fGetNearby%XYZ' AND m.definition LIKE '%@nx-(%';
GO
