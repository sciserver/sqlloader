# DR20 CSV Bulk Load Test Results

**Date**: 2026-01-12 12:13:51

**Test Mode**: First 10 rows per file

## Summary Statistics

- **Total CSV files**: 1
- **Passed validation**: 1 (100.0%)
- **Failed validation**: 0 (0.0%)
- **Total data volume**: 0.4 GB
- **Tested successfully**: 0.4 GB

## Files Passed

Total: 1 files ready for bulk loading

| Table Name | File Size | Status |
|------------|-----------|--------|
| dr20_sdss_apogeeallstarmerge_r13 | 364.7 MB | PASSED (10/10 rows) |

## Files Failed

No files failed validation! 🎉

## Generated Files

- `mssql_bulk_insert_0112.sql` - BULK INSERT statements for 1 validated files
- `test_results_0112.md` - This report

## Next Steps

1. Review generated SQL file
2. Execute full bulk load
3. Verify row counts match expected values
