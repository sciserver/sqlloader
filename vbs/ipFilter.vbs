...
'  Usage: ipFilter.vbs [<vdir_name>]
'	  (default is the www root for localhost)
'  Note: This script assumes that list of IPs to be given access
'        is in "..\etc\ipList.txt", and list of domains to be
'        given access is in "..\etc\domainList.txt".
'==============================================================
...
Dim argv, site
site = "IIS://LocalHost/W3SVC/1/ROOT/"
For Each argv In WScript.Arguments
       site = site & Lcase(argv)
Next
                                 
WScript.Echo( "Running ipFilter on URL:" & site )
Set secObj = GetObject( site )
...
Public Sub GrantIPAccess
	...
	inFileName = "..\etc\ipList.txt"
	outFileName = "C:\temp\ipLog.txt"
...
Public Sub GrantDomainAccess
	...
	inFileName = "..\etc\domainList.txt"
	outFileName = "C:\temp\domLog.txt"
	If Not fso.FileExists(inFileName) Then Fatal "Cannot find " & inFileName
	'
	Set inStream = fso.OpenTextFile(inFileName)
	Set outStream = fso.OpenTextFile(outFileName,2,True)
