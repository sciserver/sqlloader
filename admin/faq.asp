...
privileges to write files in the default (C:\sql_db\) directory.</span>
	...
	<li>
		...
		<br>
		Make sure the csvrobot.vbs script is actually being invoked.
		xp_cmdshell permissions need to be turned on, it may be off.
	...
	<li>
		<span class='t'>The CSV check step succeeds but the LOAD
		job does not pick up the task for the next step..</span>
		<br>
		Check the Servers page to make sure the current load server
		is in RUN mode for the LOAD and PUB states. If not, you will
		need to update the ServerState table in the loadadmin db to
		add or update the entries to point to the current loadserver.
	</li>
	<li>
